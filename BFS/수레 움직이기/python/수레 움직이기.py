...
def bfs(ry, rx, by, bx, maze):
    ...
    r_visited[ry][rx], b_visited[by][bx] = True, True
    queue.append([0, ry, rx, by, bx, deepcopy(r_visited), deepcopy(b_visited)])

    while queue:
        cnt, ry, rx, by, bx, r_visited, b_visited = queue.popleft()
        ...
        for dy, dx in direction:
            if maze[ry][rx] == 3:
                if boundary(by+dy, bx+dx) and not b_visited[by+dy][bx+dx] and not ((ry == by+dy) and (rx == bx+dx)) and maze[by+dy][bx+dx] != 5 :
                    ...
                    queue.append([cnt + 1, ry, rx, by+dy, bx+dx, deepcopy(r_visited), deepcopy(b_visited)])
                ...
                if boundary(ry+dy, rx+dx) and not r_visited[ry+dy][rx+dx] and not ((ry+dy == by) and (rx+dx == bx)) and maze[ry+dy][rx+dx] != 5:
                    ...
                    queue.append([cnt + 1, ry+dy, rx+dx, by, bx, deepcopy(r_visited), deepcopy(b_visited)])
                ...
                if boundary(ry+dy, rx+dx) and not r_visited[ry+dy][rx+dx] and maze[ry+dy][rx+dx] != 5:
                    for ddy, ddx in direction:
                        if boundary(by+ddy, bx+ddx) and not b_visited[by+ddy][bx+ddx] and not ((ry+dy == by+ddy) and (rx+dx == bx+ddx)) and maze[by+ddy][bx+ddx] != 5:
                            ...
                            queue.append([cnt + 1, ry+dy, rx+dx, by+ddy, bx+ddx, deepcopy(r_visited), deepcopy(b_visited)])
                
                if boundary(by+dy, bx+dx) and not b_visited[by+dy][bx+dx] and maze[by+dy][bx+dx] != 5:
                    for ddy, ddx in direction:
                        if boundary(ry+ddy, rx+ddx) and not r_visited[ry+ddy][rx+ddx] and not ((ry+ddy == by+dy) and (rx+ddx == bx+dx)) and maze[ry+ddy][rx+ddx] != 5:
                            if (ry+ddy == by and rx + ddx == bx) and (by+dy == ry and bx+dx == rx):
                                continue 
                            b_visited[by+dy][bx+dx] = True
                            r_visited[ry+ddy][rx+ddx] = True
                            queue.append([cnt + 1, ry+ddy, rx+ddx, by+dy, bx+dx, deepcopy(r_visited), deepcopy(b_visited)])
